...
[![Tests](https://img.shields.io/badge/Tests-77%20tests-success?style=for-the-badge&logo=checkmarx)](https://github.com/maatini/vetinari/actions)
...
Alternative (module form): `"args": ["run", "python", "-m", "vetinari.server"]`
...
- **Smart model routing** — Prioritizes models with matching API keys, then falls back through a configurable chain (default: Claude Sonnet 4.6 → GPT-4o-mini → DeepSeek Chat).
- **Parallel consultation** — `consult_multiple_experts` queries several experts in parallel, with aggregated `partial_success` status and a concurrency limit to reduce rate-limit pressure
- **Input limits** — configurable caps on query length, `max_tokens`, and number of experts per request (validated before any LLM call)
...
# Input limits (optional — validated before LLM calls)
MAX_QUERY_CHARS=32000
MAX_OUTPUT_TOKENS=8192
MAX_EXPERTS_PER_REQUEST=4
```
